...
ML can analyze *how scholars actually use a concept* across thousands of texts, revealing dimensions of meaning.
...
1. **Prototypical Case:** Filter for documents where `topic_4 > 0.4`. Among those, find the value closest to the **median** probability for Topic 4. Record its `doc_id`. (This is the "average" document).
2. **Extreme Case:** Find the document with the **single highest** probability in `topic_8`. Record its `doc_id`. (This is the "purest" example for this category).
3. **Boundary Case:** Find a document with `topic_6` that is **just above 0.4** (the lowest probability that still qualifies as membership). Record its `doc_id`. (This is the fuzzy edge of the category).

---

#### Task C: Close Reading (15 min)
Open the PDFs in `lab_excerpts/` that correspond to your selected `doc_id`s (e.g., `12.pdf`). Skim each, finding the parts that are unique and meaningful (legal documents have a lot of repetition and boilerplate).

For **each** document, fill out the table on the following page.
